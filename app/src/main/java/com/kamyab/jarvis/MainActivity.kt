...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun round(f:Int,s:Int=cyan,r:Float=22f)=GradientDrawable().apply{shape=GradientDrawable.RECTANGLE;setColor(f);cornerRadius=dp(r.toInt()).toFloat();setStroke(dp(1),s)}
    private fun ui(){
        ...
        root.addView(TextView(this).apply{text="PERSONAL AI ASSISTANT • v1.5 • WIDGET";textSize=10f;setTextColor(soft);gravity=Gravity.CENTER;setPadding(0,0,0,dp(14))})
        ...
        status=TextView(this).apply{text="آماده دریافت فرمان";setTextColor(soft);gravity=Gravity.CENTER;setPadding(0,dp(12),0,dp(12))};root.addView(status,ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT)
        val sc=ScrollView(this).apply{background=round(Color.rgb(8,22,34),Color.rgb(21,71,88),18f)}
        log=TextView(this).apply{text="JARVIS › v1.5 آماده است.\n";textSize=15f;setTextColor(Color.WHITE);setPadding(dp(16),dp(14),dp(16),dp(14));textDirection=View.TEXT_DIRECTION_RTL};sc.addView(log);root.addView(sc,LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,0,1f))
        val box=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL};input=EditText(this).apply{hint="مثلاً: واتساپ رو باز کن";setTextColor(Color.WHITE);setHintTextColor(Color.GRAY);setSingleLine(true)};val send=Button(this).apply{text="ارسال";setOnClickListener{val c=input.text.toString().trim();if(c.isNotEmpty()){input.setText("");process(c)}}};box.addView(input,LinearLayout.LayoutParams(0,dp(52),1f));box.addView(send,LinearLayout.LayoutParams(dp(90),dp(52)));root.addView(box)
        mic=Button(this).apply{text="🎙 میکروفون روشن — لمس برای قطع";setTextColor(bg);background=round(cyan);setOnClickListener{toggle()}};root.addView(mic,LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(52)).apply{topMargin=dp(8)});setContentView(root)
    }
    ...
    private fun app(n:String):Pair<String,String>?{val q=norm(n);if(q.isBlank())return null;val all=apps();return all.firstOrNull{norm(it.first)==q}?:all.firstOrNull{norm(it.first).contains(q)||q.contains(norm(it.first))}?:all.firstOrNull{a->aliases(q).any{key->a.second.contains(key,true)||norm(a.first).contains(norm(key))}}}
